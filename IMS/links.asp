...
rsLinks.ActiveConnection = MM_ConnIMS_STRING
rsLinks.Source = "SELECT *  FROM LINKS  WHERE L_NonProfit <> 'Y'"
...
Dim Repeat3__index
Repeat3__index = 0
rsCharity_numRows = rsCharity_numRows + Repeat3__numRows
%>
<%
...
Dim Repeat2__index
Repeat2__index = 0
rsLinks_numRows = rsLinks_numRows + Repeat2__numRows
%>
<% 
Randomize Timer
...
%>
<html>
<!-- #BeginTemplate "/Templates/IMS.dwt" --> 
<head>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" mm:layoutgroup="true">
  <tr> 
    ...
    <td width="100%" rowspan="2" valign="top" class="Content"><!-- #BeginEditable "Content" --> 
      <p><b>Links <a href="#" onClick="GP_AdvOpenWindow('/linkadd.htm','IMP','fullscreen=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes',500,400,'center','ignoreLink','',0,'');return document.MM_returnValue">- 
        Add A Link</a> </b></p>
      <p><b>Islamic Sites</b></p>
      <% 
While ((Repeat2__numRows <> 0) AND (NOT rsLinks.EOF)) 
%>
      <table width="100%" border="0" class="BoxNOB">
        <tr> 
          <td width="30%">- <a href="<%=(rsLinks.Fields.Item("L_URL").Value)%>"><%=(rsLinks.Fields.Item("L_LinkTitle").Value)%></a></td>
          <td> 
            <p><%=(rsLinks.Fields.Item("L_Description").Value)%></p>
          </td>
        </tr>
      </table>
      <% 
...
%>
      <p>&nbsp;</p>
      <p><b>Non Profit Orginisations</b></p>
      <% 
While ((Repeat3__numRows <> 0) AND (NOT rsCharity.EOF)) 
%>
      <table width="100%" border="0" class="BoxNOB">
        <tr> 
          <td width="30%">- <a href="<%=(rsCharity.Fields.Item("L_URL").Value)%>"><%=(rsCharity.Fields.Item("L_LinkTitle").Value)%></a></td>
          <td width="67%" class="BoxNOB"> 
            <p><%=(rsCharity.Fields.Item("L_Description").Value)%></p>
          </td>
        </tr>
      </table>
      <% 
